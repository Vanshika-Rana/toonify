...
const ComicBook = () => {
    ...
    const [jobId, setJobId] = useState(null); // Track the job ID
    ...
    // Loader index cycling
    useEffect(() => {
        let messageInterval;
        if (loading) {
            messageInterval = setInterval(() => {
                setLoaderIndex((prevIndex) => (prevIndex + 1) % messages.length);
            }, 10000);
        }
    ...
    useEffect(() => {
        if (jobId && polling) {
            const intervalId = setInterval(async () => {
                try {
                    const response = await fetch(`/api/proxy?jobId=${jobId}`);
    ...
    const fetchDataWithRetry = async () => {
        ...
        try {
            ...
            const result = await response.json();
            setJobId(result.jobId); // Set job ID for polling
        ...
        <>
            ...
            <div className="flex flex-col justify-center items-center min-h-screen py-8 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-indigo-900 to-slate-900">
                ...
                <p className="text-center text-indigo-200 mb-8 text-xl md:text-2xl">
                    Unleash your creativity and watch as AI brings your wildest comic ideas to life!
                </p>
                ...
                {loading && (
                    <div className="flex flex-col items-center">
                        ...
                        <p className="mt-4 text-xl font-bold text-blue-500">{messages[loaderIndex]}</p>
                    </div>
                )}
                {error && (
                    <div
